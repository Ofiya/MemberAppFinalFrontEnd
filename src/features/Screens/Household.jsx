...
const Household = () => {
    ...
    const filteredMembers = household.filter(member => {
        ...
        if(member.household?.first_name.toLowerCase().includes(search)){
            return true
        }
                ...
                <div className="p-4 flex flex-wrap gap-4">
                    <div className="flex-1 min-w-[200px]">
                        ...
                        <div className="relative">
                            <input type="text" onChange={(e) => setSearch(e.target.value)} value={search} id="household-search" placeholder="Search households..." className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                ...
                                filteredMembers.map((member) => {
                                    return(
                                        <tr className="border-b border-gray-200" key={member._id}>
                                            <td className="px-6 py-3 text-left text-sm font-normal text-gray-500 capitalize tracking-wider" >{member.household?.first_name}</td>
                                            <td className="px-6 py-3 text-left text-sm font-normal text-gray-500 capitalize tracking-wider" >{member.household?.last_name}</td>
                                        </tr>
                                    )
                                })
                            }
            ...
            <div className="mt-6 bg-white rounded-lg shadow px-4 py-3">
                <div className="flex items-center justify-between">
                    <div className="text-sm text-gray-700">
                        Showing <span className="font-medium" id="households-visible-count">{filteredMembers.length}</span> of <span className="font-medium" id="households-total-count">{household.length}</span> households
                    </div>
